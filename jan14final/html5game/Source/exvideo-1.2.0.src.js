 ...
 * @version	1.2.0
 * @updated	11 January 2019, 09:30 GMT+2
...
var ex_video_embed_is_ready           = 0;
var ex_video_embed_api_is_ready       = 0;
var ex_video_mute_override            = 0;
...
function ex_video_play(file, loop, overrideMutePolicy)
{
	...
	ex_video_mute_override = overrideMutePolicy;
	...
	if (ex_video_is_embed) {
		...
		function onPlayerReady(event) {
			...
			ex_video_loading = true;
			ex_video_embed_api_is_ready = true;
		...
		function onPlayerStateChange(event) {        
			...
			if (ex_video_embed_player.getPlayerState() === 3) {
				ex_video_embed_is_ready = 0;
			} else {
				ex_video_embed_is_ready = 1;
			}
	...
	/* Local or remote video file */
		...
		if (ex_video_interactive == false) {
			...
			videoHandle.style.pointerEvents = "auto";
		}
...
function ex_video_stop() {
	...
	if (ex_video_is_embed) {
		...
		if (ex_video_embed_api_is_ready) {
...
function ex_video_is_ready() {
	...
	if (ex_video_is_embed && ex_video_embed_player == -1) {
		return 0;
	}
	
	if (ex_video_is_embed) {
		if (ex_video_embed_api_is_ready) {
			return ex_video_embed_is_ready;
		} else {
			return 0;
		}
	} else {
